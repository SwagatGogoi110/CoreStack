...
CloudStack has first-class Testcontainers modules for every major SDK language. Each module starts a real CloudStack container before your tests run and tears it down after — no running daemon, no shared state, no port conflicts.
...
| Go | — | 🚧 In progress | — | [GitHub](https://github.com/CloudStack-io/testcontainers-CloudStack-go) |

## How it works

Every module exposes a `CloudStackContainer` class that wraps the official `CloudStack/CloudStack:latest` Docker image. When the container starts it waits for port 4566 to be ready, then exposes:

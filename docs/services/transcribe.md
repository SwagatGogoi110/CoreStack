...
CloudStack emulates a small Amazon Transcribe control-plane stub for applications
...
|---|---|---|
| `CLOUDSTACK_SERVICES_TRANSCRIBE_ENABLED` | `true` | Enable or disable the service |
...
aws transcribe start-transcription-job \
  --transcription-job-name CloudStack-demo \
...
aws transcribe create-vocabulary \
  --vocabulary-name CloudStack-vocab \
  --language-code en-US

aws transcribe list-vocabularies \
  --name-contains CloudStack
```
...
client.start_transcription_job(
    TranscriptionJobName="CloudStack-demo",
    Media={"MediaFileUri": "s3://my-bucket/audio.mp3"},
    LanguageCode="en-US",
    MediaFormat="mp3",
)

job = client.get_transcription_job(TranscriptionJobName="CloudStack-demo")

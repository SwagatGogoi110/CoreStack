...
|---|---|---|
| `CLOUDSTACK_SERVICES_CONFIGSERVICE_ENABLED` | `true` | Enable or disable the service |
...
!!! note
    Compliance status always returns `INSUFFICIENT_DATA` since CloudStack does not perform actual resource evaluation. Config rules, recorders, and conformance packs are stored and returned correctly, but no real configuration recording or compliance checking takes place.

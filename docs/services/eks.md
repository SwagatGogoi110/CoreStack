...
CloudStack starts a **k3s** (`rancher/k3s`) container for each cluster. The k3s API server is exposed on a host port from the configured range (`6500–6599`). Once `/readyz` responds, the cluster transitions to `ACTIVE` and the CA certificate is extracted from the kubeconfig.

By default `describe-cluster` returns a **host-reachable** endpoint (`https://localhost:<hostPort>`); the k3s server certificate includes a `localhost` SAN, so it verifies against the CA in `cluster.certificateAuthority.data`. Set `endpoint-mode: network` to return the container DNS name (`https://CloudStack-eks-<name>:6443`) instead — reachable from other containers on the Docker network (the pre-#1118 behaviour). In `network` mode the endpoint falls back to the host-reachable form when CloudStack runs natively, since there is no container DNS name a host client could use.
...
!!! note "Webhook reachability & networking"
    The k3s API server must be able to reach CloudStack's webhook URL. When CloudStack runs natively, k3s containers reach it via `host.docker.internal`; when CloudStack runs in a container (`CloudStack start`), CloudStack and the k3s containers share a Docker network. The k3s network is taken from `CLOUDSTACK_SERVICES_EKS_DOCKER_NETWORK` if set, otherwise the global `CLOUDSTACK_SERVICES_DOCKER_NETWORK`, otherwise the network CloudStack is itself attached to (auto-detected) — so no EKS-specific network configuration is required in the standard compose setup.
...
services:
  CloudStack:
    ...
    ports:
      - "4566:4566"
    environment:
      CLOUDSTACK_SERVICES_EKS_DOCKER_NETWORK: my_project_default
```
...
| `CLOUDSTACK_SERVICES_EKS_MOCK` | `false` | Metadata-only mode (no Docker) |
| `CLOUDSTACK_SERVICES_EKS_DEFAULT_IMAGE` | `rancher/k3s:latest` | k3s Docker image |
| `CLOUDSTACK_SERVICES_EKS_API_SERVER_BASE_PORT` | `6500` | First port in the k3s API server range |
| `CLOUDSTACK_SERVICES_EKS_API_SERVER_MAX_PORT` | `6599` | Last port in the k3s API server range |
| `CLOUDSTACK_SERVICES_EKS_DATA_PATH` | `./data/eks` | Host bind-mount root for cluster data |
| `CLOUDSTACK_SERVICES_EKS_DOCKER_NETWORK` | *(unset)* | Docker network for k3s containers (falls back to the global `CLOUDSTACK_SERVICES_DOCKER_NETWORK`, then CloudStack's own network) |
| `CLOUDSTACK_SERVICES_EKS_KEEP_RUNNING_ON_SHUTDOWN` | `false` | Leave k3s containers running after CloudStack stops |
| `CLOUDSTACK_SERVICES_EKS_ENDPOINT_MODE` | `host` | `describe-cluster` endpoint: `host` (`localhost:<hostPort>`) or `network` (container DNS) |
| `CLOUDSTACK_SERVICES_EKS_IAM_AUTH_WEBHOOK` | `true` | Wire a token-auth webhook into k3s so `aws eks get-token` works |

### Mock mode (CI / tests)

Use `CLOUDSTACK_SERVICES_EKS_MOCK=true` when you only need the API shape:
...
services:
  CloudStack:
    image: CloudStack/CloudStack:latest
    environment:
      CLOUDSTACK_SERVICES_EKS_MOCK: "true"
```

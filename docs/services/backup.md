...
```bash
CLOUDSTACK_SERVICES_BACKUP_JOB_COMPLETION_DELAY_SECONDS=3
```
...
|---|---|---|---|
| `CloudStack.services.backup.enabled` | `CLOUDSTACK_SERVICES_BACKUP_ENABLED` | `true` | Enable / disable the service |
| `CloudStack.services.backup.job-completion-delay-seconds` | `CLOUDSTACK_SERVICES_BACKUP_JOB_COMPLETION_DELAY_SECONDS` | `3` | Seconds from job start until `COMPLETED` |

...
CloudStack supports both API Gateway v1 (REST APIs) and API Gateway v2 (HTTP APIs).
...
|---|---|---|
| `CLOUDSTACK_SERVICES_APIGATEWAY_ENABLED` | `true` | Enable or disable API Gateway v1 (REST APIs) |
| `CLOUDSTACK_SERVICES_APIGATEWAYV2_ENABLED` | `true` | Enable or disable API Gateway v2 (HTTP and WebSocket APIs) |
...
CloudStack supports real WebSocket connections for API Gateway v2 WebSocket APIs. Clients connect via:
...
- **Connection URL**: CloudStack uses `ws://localhost:4566/ws/{apiId}/{stage}` instead of AWS's `wss://{api-id}.execute-api.{region}.amazonaws.com/{stage}`.

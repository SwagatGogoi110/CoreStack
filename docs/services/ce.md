...
CloudStack synthesizes Cost Explorer responses from its own resource state,
multiplied by the bundled AWS Pricing snapshot served by the
[Pricing service](pricing.md). Costs reflect what's running in CloudStack right now,
...
Each CloudStack service that wants to participate in cost reporting ships a
...
| Other CloudStack services (DDB, SQS, SNS, …) | catalog only, zero quantity | `UnpricedServicesEnumerator` |
...
| `Credit` | When `CLOUDSTACK_SERVICES_CE_CREDIT_USD_MONTHLY > 0` (see below) |
| `Tax` / `Refund` / `DiscountedUsage` / `SavingsPlan*` | Reserved for future PRs; not currently emitted |

### Synthetic credit injection

Set `CLOUDSTACK_SERVICES_CE_CREDIT_USD_MONTHLY` (default `0.0`) to emit a monthly
...
|---|---|---|
| `CLOUDSTACK_SERVICES_CE_ENABLED` | `true` | Enable or disable the service |
| `CLOUDSTACK_SERVICES_CE_CREDIT_USD_MONTHLY` | `0.0` | Synthetic monthly credit, applied as a `Credit` `RECORD_TYPE` row |

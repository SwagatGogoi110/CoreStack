...
|---|---|---|
| `CLOUDSTACK_SERVICES_KMS_ENABLED` | `true` | Enable or disable the service |
...
aws kms generate-data-key \
  ...
  --endpoint-url $AWS_ENDPOINT_URL
```
`CreateKey` also accepts a reserved creation-time tag key, `CloudStack:override-id`, when tests need a deterministic `KeyId`. CloudStack uses the tag value as the created key id, strips the reserved tag from stored resource tags, and rejects attempts to add `CloudStack:*` tags later via `TagResource`.

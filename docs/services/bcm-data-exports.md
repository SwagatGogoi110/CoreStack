...
CloudStack emulates the BCM Data Exports management plane that ships with
...
- `Export.DataQuery.QueryStatement`: required (free-form SQL — CloudStack does
...
staged as NDJSON in `CloudStack-cur-staging`, and written as Parquet by the
`CloudStack-duck` sidecar via `COPY ... TO ... (FORMAT PARQUET)`. Each
execution produces an artifact at
`s3://<S3Bucket>/<S3Prefix>/<Name>/<runId>.parquet`.

### `CLOUDSTACK_SERVICES_BCM_DATA_EXPORTS_EMIT_MODE`
...
|---|---|---|
| `CLOUDSTACK_SERVICES_BCM_DATA_EXPORTS_ENABLED` | `true` | Enable or disable the service |
| `CLOUDSTACK_SERVICES_BCM_DATA_EXPORTS_EMIT_MODE` | `synchronous` | Run mode (see above) |
...
- Custom SQL evaluation in `DataQuery.QueryStatement` — CloudStack ignores
...
- Real `RefreshCadence` scheduling beyond `SYNCHRONOUS` and the
  `daily` CloudStack-internal mode

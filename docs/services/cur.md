...
CloudStack emulates the legacy AWS Cost and Usage Report (CUR) API. Report
definitions are persisted in CloudStack's storage backend; emission produces
real Parquet artifacts in CloudStack's S3 service via the `CloudStack-duck` sidecar.
...
3. The rows are staged as newline-delimited JSON in
   `s3://CloudStack-cur-staging/cur-staging/<reportName>/<runId>.ndjson`.
4. The `CloudStack-duck` sidecar runs
   `COPY (SELECT * FROM read_json_auto(...)) TO 's3://...' (FORMAT PARQUET)`
   to produce the final Parquet object back in CloudStack S3.
5. The staging object is deleted in a best-effort `finally` block.

### `CLOUDSTACK_SERVICES_CUR_EMIT_MODE`
...
|---|---|---|
| `CLOUDSTACK_SERVICES_CUR_ENABLED` | `true` | Enable or disable the service |
| `CLOUDSTACK_SERVICES_CUR_EMIT_MODE` | `synchronous` | Run mode (see above) |
| `CLOUDSTACK_SERVICES_CUR_STAGING_BUCKET` | `CloudStack-cur-staging` | S3 bucket used to stage NDJSON before DuckDB writes Parquet |

The `CloudStack-duck` sidecar is started lazily on the first emission, the
...
- Server-side validation of S3 bucket policies on the destination
  bucket (CloudStack's S3 service still applies its own bucket-policy

...
CloudStack emulates Amazon MSK by orchestrating **Redpanda** containers. This provides high compatibility with the Kafka API while maintaining a low footprint.
...
|---|---|---|
| `CLOUDSTACK_SERVICES_MSK_ENABLED` | `true` | Enable or disable the service |
| `CLOUDSTACK_SERVICES_MSK_MOCK` | `false` | `true` = metadata-only CRUD, no Docker containers |
| `CLOUDSTACK_SERVICES_MSK_DEFAULT_IMAGE` | `redpandadata/redpanda:latest` | Docker image for Redpanda (Kafka) containers |

## How it works

When `mock` is set to `false` (default), CloudStack uses the Docker API to start a Redpanda container for each created cluster. For Docker socket setup, private registry authentication, and other Docker settings see [Docker Configuration](../configuration/docker.md).

- **Port Mapping**: The Kafka API (9092) is mapped to a dynamic host port.
- **Persistence**: Each cluster gets a named Docker volume (`CloudStack-msk-{volumeId}`). In memory mode the volume is removed on cluster delete; in persistent modes it is retained unless `CLOUDSTACK_STORAGE_PRUNE_VOLUMES_ON_DELETE=true`.

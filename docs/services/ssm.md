...
|---|---|---|
| `CLOUDSTACK_SERVICES_SSM_ENABLED` | `true` | Enable or disable the service |
| `CLOUDSTACK_SERVICES_SSM_MAX_PARAMETER_HISTORY` | `5` | Number of parameter versions retained per parameter |
| `CLOUDSTACK_STORAGE_SERVICES_SSM_MODE` | *(global default)* | Storage mode override for SSM (`memory`, `persistent`, `hybrid`, `wal`) |
| `CLOUDSTACK_STORAGE_SERVICES_SSM_FLUSH_INTERVAL_MS` | `5000` | Flush interval for `hybrid`/`wal` storage modes (milliseconds) |
...
!!! note
    `SecureString` parameters are stored as-is without actual KMS encryption in CloudStack. The type is preserved and returned correctly, but the value is not encrypted at rest.
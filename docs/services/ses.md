...
CloudStack exposes the classic Amazon SES Query API used by `aws ses ...` commands and SDKs targeting SES v1.
...
|---|---|---|
| `CLOUDSTACK_SERVICES_SES_ENABLED` | `true` | Enable or disable the SES service |
| `CLOUDSTACK_SERVICES_SES_SMTP_HOST` | *(unset)* | SMTP server host for email relay (empty = store only) |
...
services:
  CloudStack:
    ...
    ports: ["4566:4566"]
    environment:
      CLOUDSTACK_SERVICES_SES_SMTP_HOST: mailpit
      CLOUDSTACK_SERVICES_SES_SMTP_PORT: 1025
    networks: [CloudStack]
...
For test assertions and debugging, CloudStack exposes a LocalStack-compatible mailbox endpoint:
...
Messages are stored locally by CloudStack and can be persisted when SES storage is backed by persistent or hybrid storage.
...
aws ses send-email \
  ...
  --message "Subject={Data=Hello},Body={Text={Data=Sent from CloudStack SES}}" \
  --endpoint-url $AWS_ENDPOINT_URL
...
Alongside the classic Query API, CloudStack implements a subset of the SES v2 REST JSON API used by `aws sesv2 ...` commands and SDK v2 clients that target the modern SES surface.

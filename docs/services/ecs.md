...
|---|---|---|
| `CLOUDSTACK_SERVICES_ECS_ENABLED` | `true` | Enable or disable the ECS service |
| `CLOUDSTACK_SERVICES_ECS_MOCK` | `false` | Skip Docker; tasks go straight to `RUNNING` (useful for CI) |
| `CLOUDSTACK_SERVICES_ECS_DOCKER_NETWORK` | *(unset)* | Docker network for task containers |
| `CLOUDSTACK_SERVICES_ECS_DEFAULT_MEMORY_MB` | `512` | Default memory (MB) when the task definition omits it |
| `CLOUDSTACK_SERVICES_ECS_DEFAULT_CPU_UNITS` | `256` | Default CPU units when the task definition omits it |

### Mock mode

Set `CLOUDSTACK_SERVICES_ECS_MOCK=true` to run without Docker. In this mode tasks skip container launch and immediately transition to `RUNNING`, then to `STOPPED` when stopped. This is the recommended mode for unit/integration tests and CI pipelines where Docker-in-Docker is unavailable.
...
services:
  CloudStack:
    ...
    environment:
      CLOUDSTACK_SERVICES_ECS_MOCK: "true"
```
...
services:
  CloudStack:
    ...
    environment:
      CLOUDSTACK_SERVICES_ECS_MOCK: "false"
      CLOUDSTACK_SERVICES_ECS_DOCKER_NETWORK: my_network
```
...
services:
  CloudStack:
    ...
    volumes:
      - /var/run/docker.sock:/var/run/docker.sock
    environment:
      CLOUDSTACK_SERVICES_ECS_DOCKER_NETWORK: aws-local_default
```

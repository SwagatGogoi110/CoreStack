...
OpenSearch supports two modes controlled by `CLOUDSTACK_SERVICES_OPENSEARCH_MOCK`.
...
services:
  CloudStack:
    ...
    ports:
      - "4566:4566"
    environment:
      CLOUDSTACK_SERVICES_DOCKER_NETWORK: my_project_default
```
...
|---|---|---|
| `CLOUDSTACK_SERVICES_OPENSEARCH_ENABLED` | `true` | Enable/disable the service |
| `CLOUDSTACK_SERVICES_OPENSEARCH_MOCK` | `false` | `true` = metadata only (no Docker) |
| `CLOUDSTACK_SERVICES_OPENSEARCH_DEFAULT_IMAGE` | *(unset)* | Optional fixed image used for every domain regardless of `EngineVersion`. Useful for private registry mirrors. When unset, images resolve per-version from the built-in version map. |
| `CLOUDSTACK_SERVICES_OPENSEARCH_PROXY_BASE_PORT` | `9400` | Port range start for real mode |
| `CLOUDSTACK_SERVICES_OPENSEARCH_PROXY_MAX_PORT` | `9499` | Port range end for real mode |
| `CLOUDSTACK_SERVICES_OPENSEARCH_KEEP_RUNNING_ON_SHUTDOWN` | `false` | Leave containers running after CloudStack stops |
| `CLOUDSTACK_SERVICES_DOCKER_NETWORK` | *(unset)* | Shared Docker network for all container-based services including OpenSearch |
| `CLOUDSTACK_STORAGE_SERVICES_OPENSEARCH_FLUSH_INTERVAL_MS` | `5000` | Flush interval (ms) |

### Mock mode (CI / tests)

Use `CLOUDSTACK_SERVICES_OPENSEARCH_MOCK=true` when you only need the API shape:
...
services:
  CloudStack:
    image: CloudStack/CloudStack:latest
    environment:
      CLOUDSTACK_SERVICES_OPENSEARCH_MOCK: "true"
```
...
- **Instance type families:** `t3.*` / `m5/m6g/m7g.*` / `r5/r6g/r7g.*` / `c5/c6g/c7g.*` are EBS-backed (3584 GiB max). `i3.*` are local-NVMe instance-store. `or1.*` are S3-backed and surface a much larger volume ceiling on `DescribeInstanceTypeLimits` (8–36 TiB depending on size). CloudStack still boots one Docker container per domain regardless of family — the metadata fidelity matters for SDK clients that introspect, not for runtime data placement.
- **Container storage:** each domain gets a named Docker volume (`CloudStack-opensearch-{volumeId}`) created automatically. In memory mode the volume is removed on domain delete; in persistent modes it is retained unless `CLOUDSTACK_STORAGE_PRUNE_VOLUMES_ON_DELETE=true`.

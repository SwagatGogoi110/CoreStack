...
CloudStack implements the CodeBuild API — stored-state management plus real build execution inside Docker containers.
...
Source injection and artifact extraction both use the Docker API's archive copy endpoints — no bind mounts are required. This works correctly when CloudStack itself runs inside a Docker container (Docker-in-Docker).

## Buildspec Support

CloudStack parses the `buildspec.yml` embedded in the project or provided via `buildspecOverride`. Supported fields:
...
|---|---|---|
| `CLOUDSTACK_SERVICES_CODEBUILD_ENABLED` | `true` | Enable or disable the service |

...
CloudStack emulates the AWS Textract API with a dummy response stub. The response shape matches the real AWS Textract contracts so AWS SDK and CLI clients accept the reply without error. No real OCR or document analysis is performed: every call returns a fixed set of `Block` objects with synthetic metadata.
...
| `LINE` | `"CloudStack"` | CHILD → WORD |
| `WORD` | `"CloudStack"` | *(none)* |
...
|---|---|---|
| `CLOUDSTACK_SERVICES_TEXTRACT_ENABLED` | `true` | Enable or disable the service |

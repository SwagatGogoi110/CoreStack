...
CloudStack emulates the AWS Price List Service backed by a bundled static snapshot.
...
set of services and regions; for broader coverage, point CloudStack at your own
snapshot with `CLOUDSTACK_SERVICES_PRICING_SNAPSHOT_PATH`.
...
|---|---|---|
| `CLOUDSTACK_SERVICES_PRICING_ENABLED` | `true` | Enable or disable the service |
| `CLOUDSTACK_SERVICES_PRICING_SNAPSHOT_PATH` | *(unset)* | Filesystem directory overriding the bundled snapshot |

### Snapshot directory layout

When `CLOUDSTACK_SERVICES_PRICING_SNAPSHOT_PATH` is set, CloudStack reads files in this
...
Each product entry is stored as a JSON object; CloudStack re-serializes it into the

...
CloudStack manages real PostgreSQL, MySQL, and MariaDB Docker containers and proxies TCP connections to them, including IAM authentication support.
...
|---|---|---|
| `CLOUDSTACK_SERVICES_RDS_ENABLED` | `true` | Enable or disable the service |
| `CLOUDSTACK_SERVICES_RDS_PROXY_BASE_PORT` | `7000` | First host port in the RDS proxy range |
| `CLOUDSTACK_SERVICES_RDS_PROXY_MAX_PORT` | `7099` | Last host port in the RDS proxy range |
| `CLOUDSTACK_SERVICES_RDS_DEFAULT_POSTGRES_IMAGE` | `postgres:16-alpine` | Docker image for PostgreSQL instances |
| `CLOUDSTACK_SERVICES_RDS_DEFAULT_MYSQL_IMAGE` | `mysql:8.0` | Docker image for MySQL instances |
| `CLOUDSTACK_SERVICES_RDS_DEFAULT_MARIADB_IMAGE` | `mariadb:11` | Docker image for MariaDB instances |
...
services:
  CloudStack:
    ...
    environment:
      CLOUDSTACK_SERVICES_DOCKER_NETWORK: my-project_default
      CLOUDSTACK_SERVICES_RDS_PROXY_BASE_PORT: "7001"
```
...
Each DB instance and cluster gets its own named Docker volume (`CloudStack-rds-{volumeId}`) created
...
# Local dev — retain DB data across CloudStack restarts
...
!!! note "Docker Desktop on macOS"
    Named volumes work correctly on Docker Desktop for macOS. Bind mounts to paths inside the CloudStack container are not supported — use named volumes (the default).

...
CloudStack supports init hook scripts that run at defined points in the startup and shutdown lifecycle.
...
!!! tip "Use the compat image for scripts that call `aws` or `boto3`"
    ...
    Use `CloudStack/CloudStack:latest-compat` (or a pinned `x.y.z-compat`) instead of the standard image.

## Lifecycle Phases

CloudStack runs hooks in four ordered phases:
...
The `/_CloudStack/init` and `/_localstack/init` endpoints reflect each phase's completion status in real time, so external tooling can wait for `ready` before proceeding.

## Hook Directories

CloudStack merges scripts from two directory trees. The CloudStack-native tree has priority — if the same filename exists in both, the CloudStack copy runs and the LocalStack copy is skipped:

| Phase | CloudStack path | LocalStack-compat path |
|---|---|---|
| boot | `/etc/CloudStack/init/boot.d` | `/etc/localstack/init/boot.d` |
| start | `/etc/CloudStack/init/start.d` | `/etc/localstack/init/start.d` |
| ready | `/etc/CloudStack/init/ready.d` | `/etc/localstack/init/ready.d` |
| stop | `/etc/CloudStack/init/stop.d` or `/etc/CloudStack/init/shutdown.d` | `/etc/localstack/init/shutdown.d` |
...
CloudStack discovers scripts with the following extensions:
...
CloudStack uses a fail-fast strategy:
...
- A `boot` or `start`/`ready` hook failure causes CloudStack to shut down.
- A `stop` hook failure is logged but does not prevent shutdown or resource cleanup.

## AWS CLI in Hook Scripts

The compat image (`CloudStack/CloudStack:latest-compat`) includes the AWS CLI and boto3 with the local endpoint pre-configured.
...
| `AWS_CONFIG_FILE` | `/etc/CloudStack/aws/config` |
...
services:
  CloudStack:
    ...
    ports:
      - "4566:4566"
    volumes:
      - ./init/boot.d:/etc/CloudStack/init/boot.d:ro
      - ./init/start.d:/etc/CloudStack/init/start.d:ro
      - ./init/ready.d:/etc/CloudStack/init/ready.d:ro
      - ./init/stop.d:/etc/CloudStack/init/stop.d:ro
```

Phases you don't need can be omitted — CloudStack skips missing or empty directories.
...
To override individual scripts with CloudStack-specific versions while keeping the rest:

```yaml title="docker-compose.yml"
volumes:
  - ./localstack-init/ready.d:/etc/localstack/init/ready.d:ro   # existing scripts
  - ./CloudStack-init/ready.d:/etc/CloudStack/init/ready.d:ro             # overrides (take priority)
```
...
```sh title="/etc/CloudStack/init/ready.d/01-seed.sh"
#!/bin/sh
set -eu
...
```python title="/etc/CloudStack/init/ready.d/01-seed.py"
#!/usr/bin/env python3
import boto3
...
```sh title="/etc/CloudStack/init/stop.d/01-cleanup.sh"
#!/bin/sh
set -eu
...
!!! note "Shutdown timing"
    Stop hooks run before the HTTP server shuts down, so CloudStack's total shutdown time grows by
...
|---|---|---|
| `CLOUDSTACK_INIT_HOOKS_SHELL_EXECUTABLE` | `/bin/sh` | Shell used to run `.sh` scripts |
| `CLOUDSTACK_INIT_HOOKS_TIMEOUT_SECONDS` | `30` | Maximum runtime per script before it is killed and treated as a failure |
| `CLOUDSTACK_INIT_HOOKS_SHUTDOWN_GRACE_PERIOD_SECONDS` | `2` | Extra wait after terminating a timed-out script |

Example — extend the timeout for slow seed scripts:

```bash
CLOUDSTACK_INIT_HOOKS_TIMEOUT_SECONDS=120
```
...
environment:
  CLOUDSTACK_INIT_HOOKS_TIMEOUT_SECONDS: "120"
  CLOUDSTACK_INIT_HOOKS_SHELL_EXECUTABLE: /bin/bash
```

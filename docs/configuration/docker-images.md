...
CloudStack publishes images to [Docker Hub (`CloudStack/CloudStack`)](https://hub.docker.com/r/CloudStack/CloudStack).
...
|---|---|---|
| **Standard** | CloudStack native binary only | General use — CI, local dev, Testcontainers **(recommended)** |
| **Compat** | CloudStack + Python 3 + AWS CLI + boto3 | Workflows that need AWS tooling available inside the container |
...
The AWS CLI is pre-configured to talk to the local CloudStack endpoint — no `--endpoint-url` flag is needed in hook scripts:

...
CloudStack supports full per-account resource isolation out of the box. Resources created by one account are invisible to all others — no configuration flag required.

## How It Works

Every incoming request carries an AWS SigV4 `Authorization` header. CloudStack reads the **Access Key ID** (AKID) from that header and applies one simple rule:

> **If the AKID is exactly 12 digits, it is used as the account ID.**  
> Any other key format (e.g. `AKIAIOSFODNN7EXAMPLE`) falls back to `CLOUDSTACK_DEFAULT_ACCOUNT_ID`.
...
```bash
CLOUDSTACK_DEFAULT_ACCOUNT_ID=000000000000   # default
```
...
```bash
CLOUDSTACK_DEFAULT_ACCOUNT_ID=123456789012
```

## Enabling Multi-Account Isolation

Use 12-digit numeric access key IDs. The secret access key can be any non-empty string — CloudStack does not validate signatures by default.
...
CloudStack embeds the resolved account ID in every ARN it generates:
...
All services that use `StorageFactory` participate in account isolation automatically. This covers every service in CloudStack — SQS, SNS, S3, DynamoDB, Lambda, SSM, Secrets Manager, KMS, Kinesis, EventBridge, Cognito, RDS, ElastiCache, OpenSearch, MSK, and more.
...
By default CloudStack **does not** validate SigV4 signatures — only the access key ID matters for account resolution. The secret access key can be any non-empty string.

To enforce real signature validation:

```bash
CLOUDSTACK_AUTH_VALIDATE_SIGNATURES=true
CLOUDSTACK_AUTH_PRESIGN_SECRET=your-secret   # for pre-signed URL verification
```
...
Storage keys are namespaced per account at the persistence layer. When using `persistent`, `hybrid`, or `wal` storage modes, each account's data is stored under its own key prefix. Restarting CloudStack restores each account's resources independently.

## Configuration Reference

| Variable | Default | Description |
|---|---|---|
| `CLOUDSTACK_DEFAULT_ACCOUNT_ID` | `000000000000` | Account ID used when the AKID is not exactly 12 digits |
| `CLOUDSTACK_DEFAULT_REGION` | `us-east-1` | Region used when not derivable from the `Authorization` header |
| `CLOUDSTACK_AUTH_VALIDATE_SIGNATURES` | `false` | Enforce SigV4 signature verification |

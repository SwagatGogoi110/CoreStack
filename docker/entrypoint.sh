...
# ownership so the unprivileged `CloudStack` user can reach it on any host,
# then re-executes this script as `CloudStack` via gosu. The second invocation
...
# 0). Without the group fix-up below, CloudStack (uid 1001, group 0) can open
...
if [ "$(id -u)" = '0' ]; then
    if [ -S /var/run/docker.sock ]; then
        ...
        if [ "$sock_gid" != '0' ]; then
            ...
            usermod -aG "$group_name" CloudStack
        fi
    fi

    # Re-own state dir for the case where a host bind-mount arrives with
    # ownership the CloudStack user cannot write to. Ignore errors (read-only
    # mounts, unusual filesystems) so the container still starts.
    if [ -d /app/data ]; then
        chown -R CloudStack:root /app/data 2>/dev/null || true
    fi

    exec gosu CloudStack "$0" "$@"
fi

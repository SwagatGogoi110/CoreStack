...
Compatibility tests for [CloudStack](https://github.com/hectorvent/CloudStack) using the **AWS SDK for Java v2 (2.31.8)**.

Runs 313 tests across 16 test classes against a live CloudStack instance — no mocks.
...
Create a standard JUnit 5 test class in `src/test/java/com/CloudStack/test/`. Tests run against a live CloudStack instance using real AWS SDK clients.
...
```bash
docker build -t CloudStack-sdk-java .
docker run --rm --network host CloudStack-sdk-java

# Custom endpoint (macOS/Windows)
docker run --rm -e CLOUDSTACK_ENDPOINT=http://host.docker.internal:4566 CloudStack-sdk-java
```

...
public final class TestFixtures {
    ...
    static {
        String endpointStr = System.getenv("CLOUDSTACK_ENDPOINT");
    ...
    public static boolean isRealAws() {
        return "aws".equalsIgnoreCase(System.getenv("CLOUDSTACK_TARGET"));
    }
    ...
    /**
     * Get the CloudStack endpoint URI.
     */
     ...
     * <ol>
     *   <li>{@code CLOUDSTACK_S3_VHOST_ENDPOINT} env var — set this to {@code http://CloudStack:4566}
     *       in Docker test environments and point the test container's DNS at the CloudStack
     *       container so that {@code <bucket>.CloudStack} resolves via CloudStack's embedded DNS.</li>
     *   <li>{@code s3.localhost.cloudstack.io} — CloudStack's own public wildcard DNS, resolves to 127.0.0.1.</li>
    ...
    public static S3Client s3VirtualHostClient() {
        ...
        String vhostEndpoint = System.getenv("CLOUDSTACK_S3_VHOST_ENDPOINT");
     ...
     * When {@code CLOUDSTACK_S3_VHOST_ENDPOINT} is set the environment is
     * assumed to have working DNS (e.g. --dns pointing at CloudStack).
     */
    public static boolean isS3VirtualHostResolvable() {
        ...
        return isDnsResolvable("s3.localhost.cloudstack.io") || isDnsResolvable("s3.localhost.localstack.cloud");
    }

    private static String resolveVirtualHostBase(int port) {
        if (isDnsResolvable("s3.localhost.cloudstack.io")) {
            return "s3.localhost.cloudstack.io";
        }

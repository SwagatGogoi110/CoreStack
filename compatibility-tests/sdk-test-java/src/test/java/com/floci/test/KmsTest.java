...
class KmsTest {
    ...
    void tagging() {
        kms.tagResource(b -> b
                .keyId(keyId)
                .tags(software.amazon.awssdk.services.kms.model.Tag.builder().tagKey("Project").tagValue("CloudStack").build()));

        ListResourceTagsResponse tagsResponse = kms.listResourceTags(b -> b.keyId(keyId));

        assertThat(tagsResponse.tags())
                .anyMatch(t -> t.tagKey().equals("Project") && t.tagValue().equals("CloudStack"));
    }

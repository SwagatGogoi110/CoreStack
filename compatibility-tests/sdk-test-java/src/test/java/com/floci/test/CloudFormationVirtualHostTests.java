 ...
 *
 * <p>The TCP connection lands on the configured CloudStack endpoint; an execution
 * interceptor overrides the Host header so CloudStack sees the request as though it
 ...
 * name (e.g. {@code http://CloudStack:4566}), or a remote CloudStack deployment.
 */
...
class CloudFormationVirtualHostTests {
    ...
    /**
     * Rewrites the Host header on outbound requests so CloudStack sees the incoming

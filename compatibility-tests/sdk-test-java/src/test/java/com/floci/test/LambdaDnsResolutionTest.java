 ...
 *
 * <p>The endpoint used inside the Lambda must match CloudStack's CLOUDSTACK_HOSTNAME so
 * that the embedded DNS resolves it and S3VirtualHostFilter recognises the
 * virtual-host prefix. Override via the {@code CLOUDSTACK_DNS_HOSTNAME} env var
 * (default: {@code CloudStack}, matching the standard docker-compose.yml).
 */
...
class LambdaDnsResolutionTest {
    ...
    // The hostname CloudStack is reachable at from *inside* a Lambda container.
    // Must match CLOUDSTACK_HOSTNAME so the embedded DNS resolves *.{hostname} and
    // S3VirtualHostFilter extracts the bucket from the virtual-hosted Host header.
    private static final String CLOUDSTACK_DNS_ENDPOINT =
            Optional.ofNullable(System.getenv("CLOUDSTACK_DNS_HOSTNAME"))
                    .filter(h -> !h.isBlank())
                    .map(h -> h + ":4566")
                    .orElse("CloudStack:4566");
    ...
    void lambdaResolvesS3ViaVirtualHostedUrl() throws Exception {
        String payload = MAPPER.writeValueAsString(
                MAPPER.createObjectNode()
                        .put("bucket", BUCKET)
                        .put("key", KEY)
                        .put("endpoint", CLOUDSTACK_DNS_ENDPOINT));

...
Compatibility tests for [CloudStack](https://github.com/hectorvent/CloudStack) using the **AWS SDK for Go v2 (1.41.4)**.
...
```bash
docker build -t CloudStack-sdk-go .
docker run --rm --network host CloudStack-sdk-go

# Custom endpoint (macOS/Windows)
docker run --rm -e CLOUDSTACK_ENDPOINT=http://host.docker.internal:4566 CloudStack-sdk-go
```

...
Compatibility tests for [CloudStack](https://github.com/hectorvent/CloudStack) using the **AWS SDK for Rust (1.8.15)**.
...
```bash
docker build -t CloudStack-sdk-rust .
docker run --rm --network host CloudStack-sdk-rust

# Custom endpoint (macOS/Windows)
docker run --rm -e CLOUDSTACK_ENDPOINT=http://host.docker.internal:4566 CloudStack-sdk-rust
```

...
Compatibility tests for [CloudStack](https://github.com/hectorvent/CloudStack) using the **AWS CLI v2 (2.22.35)**.
...
```bash
docker build -t CloudStack-sdk-awscli .
docker run --rm --network host CloudStack-sdk-awscli

# Custom endpoint (macOS/Windows)
docker run --rm -e CLOUDSTACK_ENDPOINT=http://host.docker.internal:4566 CloudStack-sdk-awscli
```

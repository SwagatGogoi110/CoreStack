#!/usr/bin/env bats
# OpenTofu Compatibility Tests for CloudStack
...
setup_file() {
    ...
    echo "# --- tofu init ---" >&3
    run tofu init -backend-config=/tmp/CloudStack-backend.hcl \
    ...
    echo "# --- tofu plan ---" >&3
    run tofu plan -var="endpoint=${CLOUDSTACK_ENDPOINT}" -input=false -no-color
    if [ "$status" -ne 0 ]; then
    ...
    echo "# --- tofu apply ---" >&3
    run tofu apply -var="endpoint=${CLOUDSTACK_ENDPOINT}" -input=false -auto-approve -no-color
    if [ "$status" -ne 0 ]; then
...
teardown_file() {
    ...
    echo "# --- tofu destroy ---" >&3
    tofu destroy -var="endpoint=${CLOUDSTACK_ENDPOINT}" -input=false -auto-approve -no-color || true
}

#!/usr/bin/env bats
# Terraform Compatibility Tests for CloudStack
...
setup_file() {
    ...
    echo "# Endpoint: $CLOUDSTACK_ENDPOINT" >&3
    ...
    run terraform init -backend-config=/tmp/CloudStack-backend.hcl \
        -var="endpoint=${CLOUDSTACK_ENDPOINT}" -input=false -no-color
    if [ "$status" -ne 0 ]; then
    ...
    run terraform plan -var="endpoint=${CLOUDSTACK_ENDPOINT}" -input=false -no-color
    if [ "$status" -ne 0 ]; then
    ...
    run terraform apply -var="endpoint=${CLOUDSTACK_ENDPOINT}" -input=false -auto-approve -no-color
    if [ "$status" -ne 0 ]; then
...
teardown_file() {
    ...
    terraform destroy -var="endpoint=${CLOUDSTACK_ENDPOINT}" -input=false -auto-approve -no-color || true
}
...
@test "Terraform: S3 bucket created" {
    run aws_cmd s3api head-bucket --bucket CloudStack-compat-app
    assert_success
}

@test "Terraform: SQS queue created" {
    run aws_cmd sqs get-queue-url --queue-name CloudStack-compat-jobs
    assert_success
...
@test "Terraform: SNS topic created" {
    ...
    assert_success
    assert_output --partial "CloudStack-compat-events"
}

@test "Terraform: DynamoDB table created" {
    run aws_cmd dynamodb describe-table --table-name CloudStack-compat-items
    assert_success
    assert_output --partial "ACTIVE"
}

@test "Terraform: SSM parameter created" {
    run aws_cmd ssm get-parameter --name /CloudStack-compat/db-url
    assert_success
    assert_output --partial "jdbc:"
}

@test "Terraform: Secrets Manager secret created" {
    run aws_cmd secretsmanager describe-secret --secret-id "CloudStack-compat/db-creds"
    assert_success
    assert_output --partial "CloudStack-compat"
}

@test "Terraform: RDS DB instance created and available" {
    run aws_cmd rds describe-db-instances --db-instance-identifier CloudStack-compat-db
    assert_success
    assert_output --partial "CloudStack-compat-db"
    assert_output --partial "available"
}

@test "Terraform: CloudWatch alarm created with tags" {
    run aws_cmd cloudwatch describe-alarms --alarm-names CloudStack-compat-cpu-alarm
    assert_success
    assert_output --partial "CloudStack-compat-cpu-alarm"

    ALARM_ARN=$(aws_cmd cloudwatch describe-alarms --alarm-names CloudStack-compat-cpu-alarm \
...
@test "Terraform: VPC created with custom DNS settings" {
    run aws_cmd ec2 describe-vpcs \
        --filters "Name=tag:Name,Values=CloudStack-compat-vpc"
    assert_success
    assert_output --partial "CloudStack-compat-vpc"
...
@test "Terraform: Route53 hosted zone created" {
    ZONE_ID=$(aws_cmd route53 list-hosted-zones \
        --query "HostedZones[?Name=='CloudStack-compat.internal.'].Id | [0]" \
        --output text | sed 's|/hostedzone/||')
    [ -n "$ZONE_ID" ]
    run aws_cmd route53 get-hosted-zone --id "$ZONE_ID"
    assert_success
    assert_output --partial "CloudStack-compat.internal"
}
...
@test "Terraform: Route53 A record created" {
    ...
    assert_success
    assert_output --partial "app.CloudStack-compat.internal"
...
@test "Terraform: Route53 health check created" {
    HEALTH_CHECK_ID=$(aws_cmd route53 list-health-checks \
        --query "HealthChecks[?HealthCheckConfig.FullyQualifiedDomainName=='app.CloudStack-compat.internal'].Id | [0]" \
        --output text)
    [ -n "$HEALTH_CHECK_ID" ]

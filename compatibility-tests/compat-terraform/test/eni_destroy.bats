...
#
# Reproduces https://github.com/CloudStack-io/CloudStack/issues/1031
...
@test "ENI Destroy: VPC was created" {
    run aws_cmd ec2 describe-vpcs \
        --filters "Name=tag:Name,Values=CloudStack-eni-destroy-vpc"
    assert_success
    assert_output --partial "CloudStack-eni-destroy-vpc"
...
@test "ENI Destroy: Security Group was created" {
    run aws_cmd ec2 describe-security-groups \
        --group-names "CloudStack-eni-destroy-sg"
    assert_success
    assert_output --partial "CloudStack-eni-destroy-sg"
}
...
@test "ENI Destroy: terraform destroy succeeds" {
    ...
    run terraform destroy -var="endpoint=${CLOUDSTACK_ENDPOINT}" -input=false -auto-approve -no-color

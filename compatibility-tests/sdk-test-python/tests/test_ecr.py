...
REPO_NAME = "CloudStack-it/app-py"
...
class TestECRRepositoryLifecycle:
    ...
    def test_delete_force(self, ecr_client):
        ecr_client.create_repository(repositoryName="CloudStack-it/del-py")
        ecr_client.delete_repository(repositoryName="CloudStack-it/del-py", force=True)
        with pytest.raises(ClientError) as exc:
            ecr_client.describe_repositories(repositoryNames=["CloudStack-it/del-py"])
        assert exc.value.response["Error"]["Code"] == "RepositoryNotFoundException"

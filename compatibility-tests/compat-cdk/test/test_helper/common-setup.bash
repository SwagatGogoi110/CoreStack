...
export CLOUDSTACK_ENDPOINT="${CLOUDSTACK_ENDPOINT:-http://localhost:4566}"
export AWS_DEFAULT_REGION="${AWS_DEFAULT_REGION:-us-east-1}"
export AWS_ACCESS_KEY_ID="${AWS_ACCESS_KEY_ID:-test}"
export AWS_SECRET_ACCESS_KEY="${AWS_SECRET_ACCESS_KEY:-test}"
export AWS_ENDPOINT_URL="$CLOUDSTACK_ENDPOINT"

aws_cmd() {
    aws --endpoint-url "$CLOUDSTACK_ENDPOINT" --region "$AWS_DEFAULT_REGION" --output json "$@"
}
...
if [ "$LOCALSTACK_HOSTNAME" = "CloudStack" ]; then
    export CLOUDSTACK_ENDPOINT="http://CloudStack:4566"
    export AWS_ENDPOINT_URL="http://CloudStack:4566"
fi

#!/usr/bin/env bats
# CDK Compatibility Tests for CloudStack
...
@test "CDK: DynamoDB GSI exists on index table" {
    run aws_cmd dynamodb describe-table --table-name CloudStack-cdk-index-table
    assert_success
...
@test "CDK: CloudFormation stack CREATE_COMPLETE" {
    run aws_cmd cloudformation describe-stacks --stack-name CloudStackTestStack
    assert_success
...
@test "CDK: Secrets Manager generated secret exists" {
    run aws_cmd secretsmanager describe-secret --secret-id CloudStack-cdk-generated-secret
    assert_success
    secret_name=$(json_get "$output" '.Name')
    [ "$secret_name" = "CloudStack-cdk-generated-secret" ]
}

@test "CDK: GeneratedSecret username is admin" {
    run aws_cmd secretsmanager get-secret-value --secret-id CloudStack-cdk-generated-secret
    assert_success
...
@test "CDK: DockerImageFunction was created" {
    run aws_cmd lambda get-function --function-name CloudStack-cdk-docker-hello
    assert_success
    ...
    # CloudStack's Lambda launcher rewrites it to the loopback registry at pull time.
...
@test "CDK: DockerImageFunction invokes successfully" {
    ...
    # CloudStack.services.lambda.runtime-api-base-port) to be reachable from inside the
    ...
    run aws_cmd lambda invoke \
        --function-name CloudStack-cdk-docker-hello \
        --payload "$payload_b64" \
        /tmp/CloudStack-cdk-docker-hello.out
    assert_success
    status_code=$(json_get "$output" '.StatusCode')
    [ "$status_code" = "200" ]

    body=$(cat /tmp/CloudStack-cdk-docker-hello.out)
    [[ "$body" == *"Hello, world!"* ]]
    [[ "$body" == *"emulated ECR"* ]]
}

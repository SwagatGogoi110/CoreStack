# CloudStack-compatibility-tests

Compatibility test suite for [CloudStack](https://github.com/hectorvent/CloudStack) — a local AWS emulator.

Verifies that standard AWS tooling (SDKs, CDK, OpenTofu/Terraform) works correctly against the emulator without modification. Tests run against a live CloudStack instance and use real AWS SDK clients — no mocks.
...
- **CloudStack running** on `http://localhost:4566` (or set `CLOUDSTACK_ENDPOINT`)
...
```bash
CLOUDSTACK_ENDPOINT=http://localhost:4566
...
```bash
# Python
docker build -t CloudStack-sdk-python sdk-test-python/
docker run --rm --network host CloudStack-sdk-python pytest

# TypeScript
docker build -t CloudStack-sdk-node sdk-test-node/
docker run --rm --network host CloudStack-sdk-node npm test
```

On macOS/Windows, use `host.docker.internal` instead of `localhost`:

```bash
docker run --rm -e CLOUDSTACK_ENDPOINT=http://host.docker.internal:4566 CloudStack-sdk-python pytest
```

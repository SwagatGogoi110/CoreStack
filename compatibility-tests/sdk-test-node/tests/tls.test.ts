/**
 * TLS/HTTPS connectivity tests for CloudStack.
 *
 * Verifies that AWS SDK v3 clients can interact with CloudStack over HTTPS
 * when TLS is enabled with a self-signed certificate.
 *
 * These tests run within the existing test suite. Since CloudStack serves HTTP
 ...
 *
 * Prerequisites:
 *   - CloudStack must be started with CLOUDSTACK_TLS_ENABLED=true
...
// When TLS is enabled, CloudStack serves HTTPS on port 4566 (same port as HTTP without TLS).

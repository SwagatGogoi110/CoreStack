...
function wsUrl(apiId: string, stage: string): string {
  const endpoint = process.env.CLOUDSTACK_ENDPOINT || 'http://localhost:4566';
...
exports.handler = async (event) => {
  ...
  const endpoint = process.env.CLOUDSTACK_ENDPOINT + '/execute-api/' + apiId + '/' + stage;
  ...
  describe('Chat-style broadcast', () => {
    ...
    beforeAll(async () => {
      const broadcastFn = await createLambda('broadcast', BROADCAST_HANDLER, {
        CLOUDSTACK_ENDPOINT: 'http://host.docker.internal:4566',
      }, true);
